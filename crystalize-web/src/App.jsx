...
function App() {
  ...
  // NEW: Ref for File Input (to trigger from canvas)
  ...
  return (
    <div className="flex flex-col md:flex-row h-[100dvh] w-full bg-bg text-white overflow-hidden" onDrop={onDrop} onDragOver={onDragOver}>
      ...
      {showAboutModal && (
        <div className="fixed inset-0 z-[120] bg-black/80 flex items-center justify-center p-4">
            <div className="bg-panel border border-gray-700 rounded-xl p-6 w-full max-w-lg shadow-2xl relative overflow-y-auto max-h-[90vh]">
                ...
                <div className="mt-6 pt-4 border-t border-gray-700 text-center"><button onClick={() => setShowAboutModal(false)} className="px-6 py-2 bg-gray-700 hover:bg-gray-600 rounded text-white font-medium transition">Close</button></div>
            </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[110] bg-black/80 flex items-center justify-center p-4">
            <div className="bg-panel border border-gray-700 rounded-xl p-6 w-full max-w-md shadow-2xl relative">
                ...
                <div className="mb-4 space-y-3">
                    <div><label className="block text-xs font-bold text-gray-400 mb-1">Export Scale</label><div className="flex gap-2">{[1, 2, 4, 8].map(scale => (<button key={scale} onClick={() => setExportScale(scale)} className={`flex-1 py-2 rounded border font-bold text-sm transition ${exportScale === scale ? 'bg-accent border-accent text-white' : 'bg-gray-800 border-gray-700 text-gray-400 hover:bg-gray-700'}`}>{scale}x</button>))}</div></div>
      ...
      {/* SIDEBAR */}
      {/* FIXED: Added 'overflow-y-auto' so content scrolls if it's too tall (e.g. on small phones or zoomed text).
        FIXED: Added 'pb-20' or similar padding bottom to ensure last element isn't flush with edge 
      */}
      <div className={`
      ...
      `}>
        ...
        <div className="flex p-1 bg-gray-800 rounded-lg mb-6 shrink-0">
        ...
        {activeTab === 'crystals' && (
          <div className="space-y-6">
            <div className="mb-4">
                <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-600 border-dashed rounded-lg cursor-pointer hover:bg-gray-800 transition">
                    <div className="flex flex-col items-center justify-center pt-5 pb-6">
                        ...
                        {/* CHANGED: Verbiage for better mobile/desktop hybrid UX */}
                        <p className="text-sm text-gray-400">Tap or Drop Image</p>
                    </div>
                    {/* ADDED: Ref to input */}
                    <input ref={fileInputRef} type="file" className="hidden" accept="image/*" onChange={(e) => {handleFile(e.target.files[0]); setIsSidebarOpen(false);}} />
                </label>
            </div>
            ...
            <div><div className="flex justify-between mb-2"><label className="text-sm font-medium text-gray-300">Focus Details</label><span className="text-xs text-accent font-mono">{Math.round(detailBias * 100)}%</span></div><input type="range" min="0" max="1" step="0.1" value={detailBias} onChange={(e) => setDetailBias(Number(e.target.value))} className="w-full h-2 bg-gray-700 rounded-lg accent-indigo-500" /></div>
            <div><div className="flex justify-between mb-2"><label className="text-sm font-medium text-gray-300">Regularity</label><span className="text-xs text-accent font-mono">{relaxation}</span></div><input type="range" min="0" max="10" step="1" value={relaxation} onChange={(e) => setRelaxation(Number(e.target.value))} className="w-full h-2 bg-gray-700 rounded-lg accent-accent" /></div>
        ...
        <div className="mt-auto pt-6 border-t border-gray-700 flex justify-between items-center text-gray-500 shrink-0">
            <span className="text-xs">v1.0.1</span>
            <button onClick={() => { setShowAboutModal(true); setIsSidebarOpen(false); }} className="flex items-center gap-2 text-xs hover:text-accent transition"><Info size={14} /> <span>About & Privacy</span></button>
        </div>
      </div>
      ...
      >
        {/* FIX: Interactive Empty State (Click triggers upload) */}
        {!imageSrc && (
            <div 
